...
if args.dataset == 'cifar100':
    ...
    data_train = CIFAR100('data',
                          download=True,
                          transform=transform_train)
    data_test = CIFAR100('data',
                      train=False,
                      download=True,
                      transform=transform_test)
    ...
    data_test_loader = DataLoader(data_test, batch_size=128, num_workers=0)
    net = resnet.ResNet50(num_classes=100).cuda()
    criterion = torch.nn.CrossEntropyLoss().cuda()
    optimizer = torch.optim.SGD(net.parameters(), lr=0.1, momentum=0.9, weight_decay=5e-4)
...
def main():
    ...
    for e in range(1, epoch):
        train_and_test(e)
    torch.save(net.state_dict(),'teacher_resnet50_200ep'+args.dataset+'.pt')
